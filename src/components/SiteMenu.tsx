...
import { User, Mail, LogIn, Palette, Globe, ChevronLeft, X, Compass } from 'lucide-react'
...
export default function SiteMenu() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      void supabase.auth.getSession().then(({ data }) => setLoggedIn(Boolean(data.session?.user)))
    } catch {}
  }, [open])
  ...
  const items = [
    {
      href: '/journey',
      icon: Compass,
      label: locale === 'en' ? 'Knowledge journey' : locale === 'ar' ? 'رحلة المعرفة' : 'سفر دانش',
    },
    { href: '/account', icon: User, label: dict.accountTitle || 'مدیریت حساب' },
    { href: '/contact', icon: Mail, label: dict.contact || 'ارتباط با ما' },
    ...(!loggedIn ? [{ href: '/auth', icon: LogIn, label: dict.login || 'ورود' }] : []),
  ]

  const panel =
    open && mounted ? (
      <div className="fixed inset-0" style={{ zIndex: 99999 }} dir={dir}>
        <button
          type="button"
          aria-label="close"
          onClick={close}
        ...
        >
          ...
          >
            <div className="min-w-0">
              <p className="text-[15px] font-semibold">{dict.menu || 'منو'}</p>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-3 py-3">
            ...
            <button
              ...
              className="mb-3 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] font-semibold"
              style={{
                color: 'var(--text)',
                background:
                  'linear-gradient(135deg,color-mix(in srgb,var(--accent) 22%,var(--card-solid)),var(--card-solid))',
          ...
          >
            <p className="text-[11px] leading-relaxed" style={{ color: 'var(--muted)' }}>
              {dict.menuHint || 'گفتگو، نقشه ذهنی، بازی و گفتگوی جهانی از صفحه اصلی در دسترس‌اند.'}
            </p>
          </div>
        </aside>
      </div>
