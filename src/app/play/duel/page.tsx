...
const BANK: Q[] = [
  {
    id: 'd1',
    q: 'کدام گزینه بیشتر به «استدلال» نزدیک است؟',
    options: ['حفظ طوطی‌وار', 'نتیجه‌گیری از مقدمه', 'حدس تصادفی', 'تکرار شعار'],
    answer: 'نتیجه‌گیری از مقدمه',
  },
  {
    id: 'd2',
    q: 'برای یادگیری پایدار معمولاً کدام بهتر است؟',
    options: ['فقط خواندن', 'تمرین با فاصله', 'یک‌بار حفظ فشرده', 'نادیده گرفتن اشتباه'],
    answer: 'تمرین با فاصله',
  },
  {
    id: 'd3',
    q: 'سوءبرداشت یعنی…',
    options: ['نمی‌دانمِ صادقانه', 'فهم اشتباه با اطمینان نسبی', 'خستگی', 'کمبود زمان'],
    answer: 'فهم اشتباه با اطمینان نسبی',
  },
]
...
export default function DuelPage() {
  ...
  const answer = (opt: string) => {
    ...
    try {
      onQuizQuestionAnswered(`duel-${q.id}-${room}`, ok, {
        domain: 'general',
        xpCorrect: 4,
        xpWrong: 1,
      })
    ...
    <main className="min-h-screen" dir={dir} style={{ color: 'var(--text)' }}>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        <p className="text-sm text-[var(--muted)] leading-relaxed">
          {locale === 'en'
            ? 'Local duel on one device. Online realtime can be added later.'
            : 'رقابت محلی روی یک دستگاه. مسابقه آنلاین بعداً قابل افزودن است.'}
        </p>
        {!joined ? (
          <div className="card space-y-3">
            <input
              ...
              placeholder="ABC123"
            />
            <button type="button" className="btn-primary w-full py-2.5" onClick={start}>
              {locale === 'en' ? 'Start duel' : 'شروع رقابت'}
            </button>
          </div>
        ) : done ? (
          <div className="card text-center space-y-2">
            ...
            <p className="text-sm">
              {locale === 'en' ? 'You' : 'تو'}: {you} · {locale === 'en' ? 'Rival' : 'حریف'}: {rival}
            </p>
            <button type="button" className="btn-primary px-4 py-2" onClick={start}>
              {locale === 'en' ? 'Again' : 'دوباره'}
            </button>
          </div>
        ) : (
          <div className="card space-y-3">
        ...
        <Link href="/play" className="text-sm text-[var(--accent)]">
          ← {locale === 'en' ? 'Back to games' : 'بازگشت به بازی‌ها'}
        </Link>
      </div>
    </main>
  )
